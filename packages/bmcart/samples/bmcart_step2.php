...
use Packages\Bmcart\App\BmcartStep2;
use Monolog\Logger;
...
if ($is_ajax) {
    $request['usage_of_point'] = (isset($_POST['usage_of_point'])) ? $_POST['usage_of_point'] : null ;
    $request['usepoint'] = (isset($_POST['usepoint'])) ? $_POST['usepoint'] : null ;
}

$_SESSION[$cart_project]['login'] = 'k_otsuka0201';

if (isset($request['usage_of_point'])) {
    $_SESSION[$cart_project]['usage_of_point'] = $request['usage_of_point'];
}
if (isset($request['usepoint']) && $request['usepoint']) {
    $_SESSION[$cart_project]['usepoint'] = $request['usepoint'];
}
...
try {
    $bmcart = new BmcartStep2($instance_data);
    $bmcart->cartRun();
    ...
    if ($is_ajax) {
        ...
        $bmcart->template->show('mycart_step2.tpl.html');
